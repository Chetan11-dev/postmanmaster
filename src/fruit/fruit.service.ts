...
export class FruitService implements RestService<Fruit> {
    ...
    update(id: string, f: Fruit): Fruit {
        const updatedfruit = { ... this.get(id), ...f, id }
        this.remove(updatedfruit.id), updatedfruit.id
        this.add(updatedfruit, updatedfruit.id)
        return updatedfruit
    }
    ...
    add(f: Fruit, id = randomInteger().toString()): Fruit {
        const updatedFruit = { ...f, id }
